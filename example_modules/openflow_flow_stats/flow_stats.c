...
int start(oflops_context * ctx)
{
    ...
    make_ofp_flow_add(&b, fl, ctx->channels[OFLOPS_DATA3].of_port, 1, 1200);
    oflops_send_of_mesg(ctx, b);
    free(b);
...
int handle_timer_event(oflops_context * ctx, struct timer_event *te)
{
    ...
    if(strcmp(str, GETSTAT) == 0) {
        ...
        //schedule next query
        oflops_schedule_timer_event(ctx, query_delay/SEC_TO_USEC, query_delay%SEC_TO_USEC, GETSTAT);
