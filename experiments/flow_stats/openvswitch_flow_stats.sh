...
for flow_num in 1 8 64 256 1024; do
    for query_num in 1 8 64 256 1024; do
 	for query_delay in 4000000 2000000 1000000 500000 250000; do 
		...
		for table in 0 1; do 
		    if [ ! -d /testbed/data/openvswitch/flow_stats/`printf "%05d" $query_delay`/ ]; then
			mkdir /testbed/data/openvswitch/flow_stats/`printf "%05d" $query_delay`/
		    fi
			...
			mkdir /testbed/data/openvswitch/flow_stats/`printf "%05d" $query_delay`/$table_str
		    fi
			...
			dpctl del-flows tcp:192.168.1.2 #ptcp:
			sleep 20;
		    ...
		    mv oflops.log /testbed/data/openvswitch/flow_stats/`printf "%05d" $query_delay`/$table_str/`printf "%05d" $flow_num`-`printf "%05d" $query_num`-$try-oflops.log;
		    if [ -e measure.log ]; then
			...
			mv measure.log  /testbed/data/openvswitch/flow_stats/`printf "%05d" $query_delay`/$table_str/`printf "%05d" $flow_num`-`printf "%05d" $query_num`-$try-measure.log;
		    fi
		done
	    done
	done
