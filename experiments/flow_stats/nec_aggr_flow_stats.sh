 ...
 for flow_num in 1 8 64 256 1024; do
 	...
 	for query_delay in 4000000 1000000 500000 250000; do 
		...
		while [ ! -e oflops.log ] ||  [ "`wc -l oflops.log | cut -d \  -f 1 `" -lt "20" ]; do 
		    ...
		    sleep 20;
                    echo "params $query_num $flow_num $try $query_delay";
		...
		echo /testbed/data/nec/aggr_flow_stats/`printf "%05d" $query_delay`/$table_str/`printf "%05d" $flow_num`-`printf "%05d" $query_num`-$try-oflops.log;
		mv oflops.log /testbed/data/nec/aggr_flow_stats/`printf "%05d" $query_delay`/$table_str/`printf "%05d" $flow_num`-`printf "%05d" $query_num`-$try-oflops.log;
		if [ -e measure.log ]; then
		    echo /testbed/data/nec/aggr_flow_stats/`printf "%05d" $query_delay`/$table_str/`printf "%05d" $flow_num`-`printf "%05d" $query_num`-$try-measure.log;
		    mv measure.log  /testbed/data/nec/aggr_flow_stats/`printf "%05d" $query_delay`/$table_str/`printf "%05d" $flow_num`-`printf "%05d" $query_num`-$try-measure.log;
		fi
	    done
	done
    done
...
table=1;
table_str=wild;
 for flow_num in 1 8 64 1024; do
     for query_num in 1 8 64 1024; do
