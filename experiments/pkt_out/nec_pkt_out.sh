...
for try in `seq 1 20`; do 
	...
	for flow_delay in 500 1000 1500 2500 5000 10000 25000 50000 75000 100000 250000 500000 750000 1000000; do
	    ...
	    if [ ! -e /testbed/data/nec/pkt_out/`printf "%05d" $pkt_size`/ ]; then
		mkdir /testbed/data/nec/pkt_out/`printf "%05d" $pkt_size`/
	    fi 
		...
		sed -e "s/%pkt_size%/$pkt_size/g"  -e "s/%flows_delay%/$flow_delay/g" \
		    /testbed/data/nec/pkt_out/config-nec-packet-out.cfg \
		    > /tmp/oflops.cfg
	    ...
	    cp oflops.log /testbed/data/nec/pkt_out/`printf "%05d" $pkt_size`/`printf "%07d" $flow_delay`-$try-oflops.log;
	    rm oflops.log
	done
    done
done
