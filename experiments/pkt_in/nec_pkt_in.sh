...
for try in `seq 1 20`; do 
	...
	for flow_delay in 500 1000 1500 2500 5000 10000 25000 50000 75000 100000 250000 500000 750000 1000000; do
	     ...
	     mkdir /testbed/data/nec/pkt_in/`printf "%05d" $pkt_size`/
		...
		sed -e "s/%pkt_size%/$pkt_size/g"  -e "s/%flows_delay%/$flow_delay/g" \
		    /testbed/data/nec/pkt_in/config-nec-packet-in.cfg \
		    > /tmp/oflops.cfg
	    ...
	    echo /testbed/data/nec/pkt_in/`printf "%05d" $pkt_size`/`printf "%07d" $flow_delay`-$try-oflops.log;
	    cp oflops.log /testbed/data/nec/pkt_in/`printf "%05d" $pkt_size`/`printf "%07d" $flow_delay`-$try-oflops.log;
	    rm oflops.log
	done
    done
done
